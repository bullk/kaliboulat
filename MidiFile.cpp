...
{
	auto mainlog= spdlog::get( "main" );	
	mainlog->debug( "MidiFile::nameTrack {}", tn );
	...
	while ( go and (event -> size () > 0) )
	{
		...
		{
		case 0xFF:
			mainlog->debug("SMF meta : {} {}", char_vector_to_hex(*event));
			switch ( event -> at(1) )
			{
			case 0x03:
			{
				...
				mainlog->debug( "track {} : setting trackname to {}", tn, clipname );
				go = false;
				break;
			}
			default:
	...
	mainlog->debug( "parsing done" );
	
	delete event;
	mainlog->debug( "/MidiFile::nameTrack" );
...
void MidiFile::parse ()
{
	...
	//{
		...
		//{
		//case 0:
			//mainlog->debug("SMF format 0");
			//parseTrack (0);
			//break;

		//case 1:
			//mainlog->debug("SMF format 1"); // TODO : piste Master pour le format 1
			//for (unsigned int i=0; i<getNumberOfTracks(); i++) parseTrack (i);
			//break;

		//case 2:
			//mainlog->debug("SMF format 2");
			//for (unsigned int i=0; i<getNumberOfTracks(); i++) parseTrack (i);
			//break;
		//}
	//}
